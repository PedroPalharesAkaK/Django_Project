...
def popular_banco_dados(): #[cite: 2]
    ...
    caminho_arquivo = 'Nome professores fea.txt' 
    ...
    fea, _ = Instituto.objects.get_or_create(
        sigla="FEA",
        defaults={"nome": "Faculdade de Economia e Administração", "universidade": usp}
    )
    ...
    print("Iniciando a importação de docentes do fea...\n") 
    ...
    for linha in linhas: #[cite: 2]
        ...
        if len(nome) > 2 and not nome.startswith('['): #[cite: 2]
            ...
            professor, foi_criado = Professor.objects.get_or_create( #[cite: 2]
                nome=nome, #[cite: 2]
                defaults={ #[cite: 2]
                    'descricao': 'Faculdade de Economia e Administração', 
